 ...
 * [SecurityManager] to restrict [java.net.SocketPermission] to their application. By asserting that these
 * operations are privileged, the operations can proceed even if some code in the calling chain lacks the appropriate
 * [java.net.SocketPermission].
 */
object SocketUtils {

    private val EMPTY: Enumeration<Any> = Collections.enumeration(emptyList())

    @Suppress("UNCHECKED_CAST")
    private fun <T> empty(): Enumeration<T> = EMPTY as Enumeration<T>
    ...
    @JvmStatic
    @Throws(UnknownHostException::class)
    fun allAddressesByName(hostname: String): kotlin.Array<InetAddress> {
        try {
            return AccessController.doPrivileged(PrivilegedExceptionAction {
    ...
    fun addressesFromNetworkInterface(intf: NetworkInterface): Enumeration<InetAddress> {
        val addresses = AccessController.doPrivileged(PrivilegedAction {
            intf.inetAddresses
        })
